-- ==========================================
-- BASE MVP (Fases 1 a 6)
-- ==========================================

-- Função auxiliar global para triggers de updated_at
...
CREATE POLICY "Users can view their tenants" ON public.tenants FOR SELECT USING (id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view tenant_users in their tenants" ON public.tenant_users;
CREATE POLICY "Users can view tenant_users in their tenants" ON public.tenant_users FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view companies in their tenants" ON public.companies;
CREATE POLICY "Users can view companies in their tenants" ON public.companies FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage companies" ON public.companies;
CREATE POLICY "Tenant Admins and DP can manage companies" ON public.companies FOR ALL 
    USING (
...
DROP POLICY IF EXISTS "Users can view establishments in their tenants" ON public.establishments;
CREATE POLICY "Users can view establishments in their tenants" ON public.establishments FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage establishments" ON public.establishments;
CREATE POLICY "Tenant Admins and DP can manage establishments" ON public.establishments FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view departments in their tenants" ON public.departments;
CREATE POLICY "Users can view departments in their tenants" ON public.departments FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage departments" ON public.departments;
CREATE POLICY "Tenant Admins and DP can manage departments" ON public.departments FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view sectors in their tenants" ON public.sectors;
CREATE POLICY "Users can view sectors in their tenants" ON public.sectors FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage sectors" ON public.sectors;
CREATE POLICY "Tenant Admins and DP can manage sectors" ON public.sectors FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view cost centers in their tenants" ON public.cost_centers;
CREATE POLICY "Users can view cost centers in their tenants" ON public.cost_centers FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage cost centers" ON public.cost_centers;
CREATE POLICY "Tenant Admins and DP can manage cost centers" ON public.cost_centers FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view positions in their tenants" ON public.positions;
CREATE POLICY "Users can view positions in their tenants" ON public.positions FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage positions" ON public.positions;
CREATE POLICY "Tenant Admins and DP can manage positions" ON public.positions FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view workplaces in their tenants" ON public.workplaces;
CREATE POLICY "Users can view workplaces in their tenants" ON public.workplaces FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage workplaces" ON public.workplaces;
CREATE POLICY "Tenant Admins and DP can manage workplaces" ON public.workplaces FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view work schedules in their tenants" ON public.work_schedules;
CREATE POLICY "Users can view work schedules in their tenants" ON public.work_schedules FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage work schedules" ON public.work_schedules;
CREATE POLICY "Tenant Admins and DP can manage work schedules" ON public.work_schedules FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view holidays in their tenants" ON public.holidays;
CREATE POLICY "Users can view holidays in their tenants" ON public.holidays FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage holidays" ON public.holidays;
CREATE POLICY "Tenant Admins and DP can manage holidays" ON public.holidays FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view contract types in their tenants" ON public.contract_types;
CREATE POLICY "Users can view contract types in their tenants" ON public.contract_types FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage contract types" ON public.contract_types;
CREATE POLICY "Tenant Admins and DP can manage contract types" ON public.contract_types FOR ALL 
    USING (tenant_id = ANY (public.user_tenant_ids()))
    WITH CHECK (tenant_id = ANY (public.user_tenant_ids()));
...
DROP POLICY IF EXISTS "Auditors can read tenant logs" ON audit.logs;
CREATE POLICY "Auditors can read tenant logs" ON audit.logs FOR SELECT 
    USING (
        tenant_id = ANY (public.user_tenant_ids())
        AND EXISTS (
            SELECT 1 FROM public.tenant_users 
            ...
            AND role IN ('system_admin', 'tenant_admin', 'auditor')
        )
    );
...
DROP TRIGGER IF EXISTS audit_establishments_trigger ON public.establishments;
DROP TRIGGER IF EXISTS audit_establishments_trigger ON public.establishments;
...
-- 5.1 Salary History
CREATE TABLE IF NOT EXISTS public.contract_salary_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.contract_position_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
    contract_id UUID NOT NULL REFERENCES public.employment_contracts(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.contract_department_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
    contract_id UUID NOT NULL REFERENCES public.employment_contracts(id) ON DELETE CASCADE,
...
DROP POLICY IF EXISTS "Users can view people in their tenants" ON public.people;
CREATE POLICY "Users can view people in their tenants" ON public.people FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view workers in their tenants" ON public.workers;
CREATE POLICY "Users can view workers in their tenants" ON public.workers FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view employment_contracts in their tenants" ON public.employment_contracts;
CREATE POLICY "Users can view employment_contracts in their tenants" ON public.employment_contracts FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage people" ON public.people;
CREATE POLICY "Tenant Admins and DP can manage people" ON public.people FOR ALL USING (
...
CREATE POLICY "Tenant Admins and DP can manage workers" ON public.workers FOR ALL USING (
...
CREATE POLICY "Tenant Admins and DP can manage employment_contracts" ON public.employment_contracts FOR ALL USING (
...
DROP POLICY IF EXISTS "Users can view bank_accounts in their tenants" ON public.bank_accounts;
CREATE POLICY "Users can view bank_accounts in their tenants" ON public.bank_accounts FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage bank_accounts" ON public.bank_accounts;
CREATE POLICY "Tenant Admins and DP can manage bank_accounts" ON public.bank_accounts FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Users can view history tables in their tenants" ON public.contract_salary_history;
CREATE POLICY "Users can view history tables in their tenants" ON public.contract_salary_history FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));
DROP POLICY IF EXISTS "Users can view position history in their tenants" ON public.contract_position_history;
CREATE POLICY "Users can view position history in their tenants" ON public.contract_position_history FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));
DROP POLICY IF EXISTS "Users can view department history in their tenants" ON public.contract_department_history;
CREATE POLICY "Users can view department history in their tenants" ON public.contract_department_history FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));

DROP POLICY IF EXISTS "Tenant Admins and DP can manage history tables" ON public.contract_salary_history;
CREATE POLICY "Tenant Admins and DP can manage history tables" ON public.contract_salary_history FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage position history" ON public.contract_position_history;
CREATE POLICY "Tenant Admins and DP can manage position history" ON public.contract_position_history FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage department history" ON public.contract_department_history;
CREATE POLICY "Tenant Admins and DP can manage department history" ON public.contract_department_history FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()));

DROP TRIGGER IF EXISTS audit_people_trigger ON public.people;
DROP TRIGGER IF EXISTS audit_people_trigger ON public.people;
...
DROP TRIGGER IF EXISTS audit_contracts_trigger ON public.employment_contracts;
DROP TRIGGER IF EXISTS audit_contracts_trigger ON public.employment_contracts;
...
CREATE TABLE IF NOT EXISTS public.payroll_rubrics (
    ...
    name TEXT NOT NULL,
    esocial_code TEXT,
    rubric_type TEXT NOT NULL CHECK (rubric_type IN ('EARNING', 'DEDUCTION', 'NEUTRAL', 'BASE')),
...
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS category TEXT;
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS calculation_type TEXT;
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS calculation_form TEXT;
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS calculation_base TEXT;
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS factor NUMERIC(10,4);
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS formula TEXT;
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS generates_inss_base BOOLEAN DEFAULT false;
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS generates_irrf_base BOOLEAN DEFAULT false;
ALTER TABLE public.payroll_rubrics ADD COLUMN IF NOT EXISTS generates_fgts_base BOOLEAN DEFAULT false;
ALTER TABLE public.payroll_rubrics DROP CONSTRAINT IF EXISTS payroll_rubrics_rubric_type_check;
ALTER TABLE public.payroll_rubrics ADD CONSTRAINT payroll_rubrics_rubric_type_check CHECK (rubric_type IN ('EARNING', 'DEDUCTION', 'NEUTRAL', 'BASE'));
...
ALTER TABLE public.benefit_catalogs ADD COLUMN IF NOT EXISTS rubric_id UUID REFERENCES public.payroll_rubrics(id);
ALTER TABLE public.benefit_catalogs ADD COLUMN IF NOT EXISTS discount_type TEXT;
ALTER TABLE public.benefit_catalogs ADD COLUMN IF NOT EXISTS default_discount_value NUMERIC(10,2);
...
BEGIN
    FOR tenant IN SELECT id FROM public.tenants LOOP
        ...
        INSERT INTO public.payroll_rubrics (
            tenant_id, code, name, rubric_type, category, calculation_type, calculation_form, calculation_base, factor, 
        ...
        INSERT INTO public.payroll_rubrics (
            tenant_id, code, name, rubric_type, category, calculation_type, calculation_form, formula, 
            incidence_inss, incidence_irrf, incidence_fgts, generates_inss_base, generates_irrf_base, generates_fgts_base
        ) VALUES (
...
BEGIN
    FOR tenant IN SELECT id FROM public.tenants LOOP
        ...
        INSERT INTO public.payroll_rubrics (
            tenant_id, code, name, rubric_type, category, calculation_type, calculation_form, calculation_base, 
            incidence_inss, incidence_irrf, incidence_fgts, generates_inss_base, generates_irrf_base, generates_fgts_base
        ) VALUES (
        ...
        UPDATE public.benefit_catalogs 
...
DROP POLICY IF EXISTS "Users can view onboarding in their tenants" ON public.onboarding_processes;
...
DROP POLICY IF EXISTS "Users can view onboarding docs in their tenants" ON public.onboarding_documents;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage onboarding" ON public.onboarding_processes;
CREATE POLICY "Tenant Admins and DP can manage onboarding" ON public.onboarding_processes FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.onboarding_processes.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage onboarding docs" ON public.onboarding_documents;
CREATE POLICY "Tenant Admins and DP can manage onboarding docs" ON public.onboarding_documents FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.onboarding_documents.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));

DROP TRIGGER IF EXISTS audit_onboarding_processes_trigger ON public.onboarding_processes;
DROP TRIGGER IF EXISTS audit_onboarding_processes_trigger ON public.onboarding_processes;
...
DROP TRIGGER IF EXISTS audit_onboarding_documents_trigger ON public.onboarding_documents;
DROP TRIGGER IF EXISTS audit_onboarding_documents_trigger ON public.onboarding_documents;
...
CREATE TABLE IF NOT EXISTS public.signatures_log (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
DROP POLICY IF EXISTS "Users can view templates in their tenants" ON public.document_templates;
...
DROP POLICY IF EXISTS "Users can view employee documents in their tenants" ON public.employee_documents;
...
DROP POLICY IF EXISTS "Users can view signature logs in their tenants" ON public.signatures_log;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage templates" ON public.document_templates;
CREATE POLICY "Tenant Admins and DP can manage templates" ON public.document_templates FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.document_templates.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage employee documents" ON public.employee_documents;
CREATE POLICY "Tenant Admins and DP can manage employee documents" ON public.employee_documents FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.employee_documents.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));
DROP POLICY IF EXISTS "System generates signature logs" ON public.signatures_log;
...
DROP TRIGGER IF EXISTS audit_templates_trigger ON public.document_templates;
DROP TRIGGER IF EXISTS audit_templates_trigger ON public.document_templates;
...
DROP TRIGGER IF EXISTS audit_employee_docs_trigger ON public.employee_documents;
DROP TRIGGER IF EXISTS audit_employee_docs_trigger ON public.employee_documents;
...
ALTER TABLE public.employment_contracts ADD COLUMN IF NOT EXISTS union_id UUID REFERENCES public.unions(id);

CREATE TABLE IF NOT EXISTS public.vacation_vesting_periods (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
    contract_id UUID NOT NULL REFERENCES public.employment_contracts(id) ON DELETE CASCADE,
    start_date DATE NOT NULL,
...
DROP POLICY IF EXISTS "Users can view unions" ON public.unions;
CREATE POLICY "Users can view unions" ON public.unions FOR SELECT USING (tenant_id = ANY (public.user_tenant_ids()));
DROP POLICY IF EXISTS "Users can view collective agreements" ON public.collective_agreements;
...
DROP POLICY IF EXISTS "Users can view vacation vesting" ON public.vacation_vesting_periods;
...
DROP POLICY IF EXISTS "Users can view vacation requests" ON public.vacation_requests;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage unions" ON public.unions;
CREATE POLICY "Tenant Admins and DP can manage unions" ON public.unions FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.unions.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage collective agreements" ON public.collective_agreements;
CREATE POLICY "Tenant Admins and DP can manage collective agreements" ON public.collective_agreements FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.collective_agreements.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));

DROP TRIGGER IF EXISTS audit_unions_trigger ON public.unions;
DROP TRIGGER IF EXISTS audit_unions_trigger ON public.unions;
...
DROP TRIGGER IF EXISTS audit_vacations_trigger ON public.vacation_requests;
DROP TRIGGER IF EXISTS audit_vacations_trigger ON public.vacation_requests;
...
ALTER TABLE public.employment_contracts ADD COLUMN IF NOT EXISTS work_schedule_id UUID REFERENCES public.work_schedules(id);
...
DROP POLICY IF EXISTS "Users can view benefits" ON public.benefit_catalogs;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage leaves" ON public.leaves;
CREATE POLICY "Tenant Admins and DP can manage leaves" ON public.leaves FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.leaves.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage benefits" ON public.benefit_catalogs;
CREATE POLICY "Tenant Admins and DP can manage benefits" ON public.benefit_catalogs FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.benefit_catalogs.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage employee benefits" ON public.employee_benefits;
CREATE POLICY "Tenant Admins and DP can manage employee benefits" ON public.employee_benefits FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.employee_benefits.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage work schedules" ON public.work_schedules;
CREATE POLICY "Tenant Admins and DP can manage work schedules" ON public.work_schedules FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.work_schedules.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));

DROP TRIGGER IF EXISTS audit_leaves_trigger ON public.leaves;
DROP TRIGGER IF EXISTS audit_leaves_trigger ON public.leaves;
...
DROP TRIGGER IF EXISTS audit_employee_benefits_trigger ON public.employee_benefits;
DROP TRIGGER IF EXISTS audit_employee_benefits_trigger ON public.employee_benefits;
...
DROP TRIGGER IF EXISTS audit_work_schedules_trigger ON public.work_schedules;
DROP TRIGGER IF EXISTS audit_work_schedules_trigger ON public.work_schedules;
...
CREATE OR REPLACE RULE prevent_time_entry_deletion AS ON DELETE TO public.time_entries DO INSTEAD NOTHING;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage time" ON public.time_entries;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage timesheets" ON public.timesheets;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage payroll" ON public.payroll_periods;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage rubrics" ON public.payroll_rubrics;
...
CREATE TABLE IF NOT EXISTS public.payroll_event_memory_calc (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
ALTER TABLE public.payroll_event_memory_calc ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.terminations ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Tenant Admins and DP can manage progressive tables" ON public.payroll_progressive_tables;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage payroll events" ON public.payroll_events;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can view memory calc" ON public.payroll_event_memory_calc;
CREATE POLICY "Tenant Admins and DP can view memory calc" ON public.payroll_event_memory_calc FOR SELECT USING (EXISTS (SELECT 1 FROM public.payroll_events e WHERE e.id = public.payroll_event_memory_calc.event_id AND e.tenant_id = ANY(public.user_tenant_ids())));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage terminations" ON public.terminations;
...
DROP TRIGGER IF EXISTS audit_progressive_tables_trigger ON public.payroll_progressive_tables;
DROP TRIGGER IF EXISTS audit_progressive_tables_trigger ON public.payroll_progressive_tables;
...
DROP TRIGGER IF EXISTS audit_payroll_events_trigger ON public.payroll_events;
DROP TRIGGER IF EXISTS audit_payroll_events_trigger ON public.payroll_events;
...
DROP TRIGGER IF EXISTS audit_terminations_trigger ON public.terminations;
DROP TRIGGER IF EXISTS audit_terminations_trigger ON public.terminations;
...
CREATE TABLE IF NOT EXISTS public.esocial_transmissions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.sst_work_environments (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
    name TEXT NOT NULL,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS public.sst_risks (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.sst_health_exams (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
    contract_id UUID NOT NULL REFERENCES public.employment_contracts(id) ON DELETE CASCADE,
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage eSocial" ON public.esocial_transmissions;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage SST Envs" ON public.sst_work_environments;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage SST Risks" ON public.sst_risks;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage Health Exams" ON public.sst_health_exams;
...
DROP TRIGGER IF EXISTS audit_esocial_trigger ON public.esocial_transmissions;
DROP TRIGGER IF EXISTS audit_esocial_trigger ON public.esocial_transmissions;
...
DROP TRIGGER IF EXISTS audit_sst_exams_trigger ON public.sst_health_exams;
DROP TRIGGER IF EXISTS audit_sst_exams_trigger ON public.sst_health_exams;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage time adjustments" ON public.time_entry_adjustments;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage lgpd consents" ON public.lgpd_consents;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage retention policies" ON public.data_retention_policies;
...
DROP TRIGGER IF EXISTS audit_contract_history_trigger ON public.employment_contract_history;
DROP TRIGGER IF EXISTS audit_contract_history_trigger ON public.employment_contract_history;
...
DROP TRIGGER IF EXISTS audit_time_adjustments_trigger ON public.time_entry_adjustments;
DROP TRIGGER IF EXISTS audit_time_adjustments_trigger ON public.time_entry_adjustments;
...
DROP TRIGGER IF EXISTS audit_lgpd_consents_trigger ON public.lgpd_consents;
DROP TRIGGER IF EXISTS audit_lgpd_consents_trigger ON public.lgpd_consents;
...
-- ==========================================
-- SCRIPT: supabase_sql_fase_7.sql
-- ==========================================
...
-- Políticas de RLS
DROP POLICY IF EXISTS "Users can view leaves from their tenants" ON public.leaves;
CREATE POLICY "Users can view leaves from their tenants" ON public.leaves FOR SELECT 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can insert leaves into their tenants" ON public.leaves;
CREATE POLICY "Users can insert leaves into their tenants" ON public.leaves FOR INSERT 
    WITH CHECK (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can update leaves in their tenants" ON public.leaves;
CREATE POLICY "Users can update leaves in their tenants" ON public.leaves FOR UPDATE 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can delete leaves in their tenants" ON public.leaves;
CREATE POLICY "Users can delete leaves in their tenants" ON public.leaves FOR DELETE 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
...
-- ==========================================
-- SCRIPT: supabase_sql_fase_8.sql
-- ==========================================
...
-- ==========================================
-- SCRIPT: supabase_sql_fase_9.sql
-- ==========================================
...
BEGIN
    BEGIN
        ALTER TABLE public.payroll_events ADD COLUMN IF NOT EXISTS quantity NUMERIC(10,2);
    EXCEPTION WHEN duplicate_column THEN END;

    BEGIN
        ALTER TABLE public.payroll_events ADD COLUMN IF NOT EXISTS origin TEXT NOT NULL DEFAULT 'MANUAL' CHECK (origin IN ('MANUAL', 'PONTO', 'BENEFICIO', 'INTEGRACAO'));
    EXCEPTION WHEN duplicate_column THEN END;

    BEGIN
        ALTER TABLE public.payroll_events ADD COLUMN IF NOT EXISTS notes TEXT;
    EXCEPTION WHEN duplicate_column THEN END;

    BEGIN
        ALTER TABLE public.payroll_events ADD COLUMN IF NOT EXISTS recorded_by UUID;
    EXCEPTION WHEN duplicate_column THEN END;
END $$;
...
-- ==========================================
-- SCRIPT: supabase_sql_fase_10.sql
-- ==========================================
...
DROP POLICY IF EXISTS "Users can view benefit catalogs from their tenants" ON public.benefit_catalogs;
CREATE POLICY "Users can view benefit catalogs from their tenants" ON public.benefit_catalogs FOR SELECT 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can insert benefit catalogs into their tenants" ON public.benefit_catalogs;
CREATE POLICY "Users can insert benefit catalogs into their tenants" ON public.benefit_catalogs FOR INSERT 
    WITH CHECK (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can update benefit catalogs in their tenants" ON public.benefit_catalogs;
CREATE POLICY "Users can update benefit catalogs in their tenants" ON public.benefit_catalogs FOR UPDATE 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can delete benefit catalogs in their tenants" ON public.benefit_catalogs;
CREATE POLICY "Users can delete benefit catalogs in their tenants" ON public.benefit_catalogs FOR DELETE 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

-- Políticas de RLS para employee_benefits
DROP POLICY IF EXISTS "Users can view employee benefits from their tenants" ON public.employee_benefits;
CREATE POLICY "Users can view employee benefits from their tenants" ON public.employee_benefits FOR SELECT 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can insert employee benefits into their tenants" ON public.employee_benefits;
CREATE POLICY "Users can insert employee benefits into their tenants" ON public.employee_benefits FOR INSERT 
    WITH CHECK (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can update employee benefits in their tenants" ON public.employee_benefits;
CREATE POLICY "Users can update employee benefits in their tenants" ON public.employee_benefits FOR UPDATE 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can delete employee benefits in their tenants" ON public.employee_benefits;
CREATE POLICY "Users can delete employee benefits in their tenants" ON public.employee_benefits FOR DELETE 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
...
-- ==========================================
-- SCRIPT: supabase_sql_fase_11.sql
-- ==========================================
...
DROP POLICY IF EXISTS "Users can view employee deductions from their tenants" ON public.employee_deductions;
CREATE POLICY "Users can view employee deductions from their tenants" ON public.employee_deductions FOR SELECT 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can insert employee deductions into their tenants" ON public.employee_deductions;
CREATE POLICY "Users can insert employee deductions into their tenants" ON public.employee_deductions FOR INSERT 
    WITH CHECK (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can update employee deductions in their tenants" ON public.employee_deductions;
CREATE POLICY "Users can update employee deductions in their tenants" ON public.employee_deductions FOR UPDATE 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

DROP POLICY IF EXISTS "Users can delete employee deductions in their tenants" ON public.employee_deductions;
CREATE POLICY "Users can delete employee deductions in their tenants" ON public.employee_deductions FOR DELETE 
    USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
...
-- ==========================================
-- SCRIPT: supabase_sql_fase_12.sql
-- ==========================================
...
-- legal_versions
DROP POLICY IF EXISTS "Allow authenticated read on legal_versions" ON public.legal_versions;
CREATE POLICY "Allow authenticated read on legal_versions" ON public.legal_versions FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow authenticated insert on legal_versions" ON public.legal_versions;
CREATE POLICY "Allow authenticated insert on legal_versions" ON public.legal_versions FOR INSERT TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "Allow authenticated update on legal_versions" ON public.legal_versions;
CREATE POLICY "Allow authenticated update on legal_versions" ON public.legal_versions FOR UPDATE TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow authenticated delete on legal_versions" ON public.legal_versions;
CREATE POLICY "Allow authenticated delete on legal_versions" ON public.legal_versions FOR DELETE TO authenticated USING (true);

-- inss_parameters
DROP POLICY IF EXISTS "Allow authenticated read on inss_parameters" ON public.inss_parameters;
CREATE POLICY "Allow authenticated read on inss_parameters" ON public.inss_parameters FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow authenticated insert on inss_parameters" ON public.inss_parameters;
CREATE POLICY "Allow authenticated insert on inss_parameters" ON public.inss_parameters FOR INSERT TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "Allow authenticated update on inss_parameters" ON public.inss_parameters;
CREATE POLICY "Allow authenticated update on inss_parameters" ON public.inss_parameters FOR UPDATE TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow authenticated delete on inss_parameters" ON public.inss_parameters;
CREATE POLICY "Allow authenticated delete on inss_parameters" ON public.inss_parameters FOR DELETE TO authenticated USING (true);

-- irrf_parameters
DROP POLICY IF EXISTS "Allow authenticated read on irrf_parameters" ON public.irrf_parameters;
CREATE POLICY "Allow authenticated read on irrf_parameters" ON public.irrf_parameters FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow authenticated insert on irrf_parameters" ON public.irrf_parameters;
CREATE POLICY "Allow authenticated insert on irrf_parameters" ON public.irrf_parameters FOR INSERT TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "Allow authenticated update on irrf_parameters" ON public.irrf_parameters;
CREATE POLICY "Allow authenticated update on irrf_parameters" ON public.irrf_parameters FOR UPDATE TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow authenticated delete on irrf_parameters" ON public.irrf_parameters;
CREATE POLICY "Allow authenticated delete on irrf_parameters" ON public.irrf_parameters FOR DELETE TO authenticated USING (true);

-- general_legal_parameters
DROP POLICY IF EXISTS "Allow authenticated read on general_legal_parameters" ON public.general_legal_parameters;
CREATE POLICY "Allow authenticated read on general_legal_parameters" ON public.general_legal_parameters FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow authenticated insert on general_legal_parameters" ON public.general_legal_parameters;
CREATE POLICY "Allow authenticated insert on general_legal_parameters" ON public.general_legal_parameters FOR INSERT TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "Allow authenticated update on general_legal_parameters" ON public.general_legal_parameters;
CREATE POLICY "Allow authenticated update on general_legal_parameters" ON public.general_legal_parameters FOR UPDATE TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow authenticated delete on general_legal_parameters" ON public.general_legal_parameters;
CREATE POLICY "Allow authenticated delete on general_legal_parameters" ON public.general_legal_parameters FOR DELETE TO authenticated USING (true);
...
-- ==========================================
-- SCRIPT: supabase_sql_fase_13_14.sql
-- ==========================================
...
-- payroll_periods
DROP POLICY IF EXISTS "Users can view payroll_periods from their tenants" ON public.payroll_periods;
CREATE POLICY "Users can view payroll_periods from their tenants" ON public.payroll_periods FOR SELECT USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage payroll_periods" ON public.payroll_periods;
CREATE POLICY "Tenant Admins and DP can manage payroll_periods" ON public.payroll_periods FOR ALL USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

-- payroll_variable_events
DROP POLICY IF EXISTS "Users can view payroll_variable_events from their tenants" ON public.payroll_variable_events;
CREATE POLICY "Users can view payroll_variable_events from their tenants" ON public.payroll_variable_events FOR SELECT USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage payroll_variable_events" ON public.payroll_variable_events;
CREATE POLICY "Tenant Admins and DP can manage payroll_variable_events" ON public.payroll_variable_events FOR ALL USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

-- payslips
DROP POLICY IF EXISTS "Users can view payslips from their tenants" ON public.payslips;
CREATE POLICY "Users can view payslips from their tenants" ON public.payslips FOR SELECT USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage payslips" ON public.payslips;
CREATE POLICY "Tenant Admins and DP can manage payslips" ON public.payslips FOR ALL USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

-- payslip_items
DROP POLICY IF EXISTS "Users can view payslip_items from their tenants" ON public.payslip_items;
CREATE POLICY "Users can view payslip_items from their tenants" ON public.payslip_items FOR SELECT USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage payslip_items" ON public.payslip_items;
CREATE POLICY "Tenant Admins and DP can manage payslip_items" ON public.payslip_items FOR ALL USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));

-- payroll_memory_calc
DROP POLICY IF EXISTS "Users can view payroll_memory_calc from their tenants" ON public.payroll_memory_calc;
CREATE POLICY "Users can view payroll_memory_calc from their tenants" ON public.payroll_memory_calc FOR SELECT USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
DROP POLICY IF EXISTS "Tenant Admins and DP can manage payroll_memory_calc" ON public.payroll_memory_calc;
CREATE POLICY "Tenant Admins and DP can manage payroll_memory_calc" ON public.payroll_memory_calc FOR ALL USING (tenant_id IN (SELECT unnest(public.user_tenant_ids())));
...
-- ==========================================
-- SCRIPT: supabase_sql_fase_15_rbac.sql
-- ==========================================
...
CREATE POLICY "Users can view profiles in their tenants" ON public.user_profiles FOR SELECT 
    USING (
        id IN (
...
-- Deferred Dependents constraints and policies
ALTER TABLE public.dependents ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dependents ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users can view dependents in their tenants" ON public.dependents;
...
DROP POLICY IF EXISTS "Tenant Admins and DP can manage dependents" ON public.dependents;
CREATE POLICY "Tenant Admins and DP can manage dependents" ON public.dependents FOR ALL USING (tenant_id = ANY (public.user_tenant_ids()) AND EXISTS (SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = public.dependents.tenant_id AND role IN ('system_admin', 'tenant_admin', 'dp_analyst')));
